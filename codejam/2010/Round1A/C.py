...
def main():
    ...
    for case_index in xrange(T):
        ...
        for a in xrange(A1, A2+1):
            for alpha in xrange(0, a):
                print a, alpha
                if B1 <= alpha <= B2 and check_win(a, alpha):
                    win_count += 1
                if B1 <= a+alpha <= B2 and check_win(a+alpha, a):
                    win_count += 1
                if B1 <= 2*a+alpha <= B2 and check_win(2*a+alpha, a):
                    win_count += (int(B2/a)-2+1)
